#!/usr/bin/env python3
"""Fetches Wikipedia's "Vital Articles" Level 1, 2, 3 and 4 lists and writes
a deduplicated, alphabetized JSON array of article titles suitable for
wikiman's curated title pool. Standard library only.

Levels 1-3 each live on a single page. Level 4 is split across topical
subpages (People, History, Geography, ...), so those are discovered by
reading the Level 4 index page rather than hardcoded.
...
REQUEST_DELAY_SECONDS = 10.0 # Delay between requests to avoid rate limiters
...
LEVEL_4_INDEX = "Wikipedia:Vital articles/Level 4"
LEVEL_4_SUBPAGE_PATTERN = re.compile(
    r"^Wikipedia:Vital articles/Level 4/[^|\]]+", re.IGNORECASE
)
...
def find_level_4_subpages(index_wikitext):
    subpages = []
    for match in LINK_PATTERN.finditer(index_wikitext):
        target = match.group(1).split("|")[0].split("#")[0].strip()
        if LEVEL_4_SUBPAGE_PATTERN.match(target):
...
def main():
    ...
    print(f"Fetching {LEVEL_4_INDEX}...", file=sys.stderr)
    index_wikitext = fetch_wikitext(LEVEL_4_INDEX)
    subpages = find_level_4_subpages(index_wikitext)
    print(f"Found {len(subpages)} Level 4 subpages.", file=sys.stderr)
    time.sleep(REQUEST_DELAY_SECONDS)

    for subpage in subpages:
        print(f"Fetching {subpage}...", file=sys.stderr)
